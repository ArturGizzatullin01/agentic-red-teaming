...
- [X] T036a [US2] FIX-10: показать судейский провенанс в
      `src/memnotsafe/reporting/html_report.py` ФАКТИЧЕСКИ. T036 был отмечен выполненным, но
      модуль писал «Judge inactive» безусловно — даже когда судья выносил вердикты (аудит E1/H3)
  - **Given** прогон, где судья оценил стадии `retrieval`, `adoption` и `external_effect`
  - **When** собран `report.html` — из прогона или из сохранённого `campaign.json`
  - **Then** по каждой судейской стадии видны исход, модель, рубрика, уверенность, цитата и
    `artifact_ref`, а при расхождении — оба вердикта со своими формулировками причин (FR-008)
  - **And** `outcome == "unavailable"` читается как «судья не ответил» с записанной причиной,
    а не как «судьи не было»: это разные факты (FR-020)
  - **And** случай без единого вердикта не выдаётся за оценённый, даже когда судья активен в
    кампании: модель и рубрика прогона такому случаю не приписываются
  - **And** цитата, обоснование, модель и рубрика экранируются — текст приходит из враждебного
    источника; собственный `<script>` фильтров отчёта при этом сохраняется
  - Файлы: `src/memnotsafe/reporting/html_report.py`, `tests/test_reporting_html.py`
  - Проверки (`.agent-work/reporter-venv`, `PYTHONPATH=src`): RED — 9 из 10 новых тестов красные
    до правки; `tests/test_reporting_html.py tests/test_reporting_replay.py` → 30 passed;
    `tests/test_e2e_cross_user.py tests/test_all_attacks.py` → 9 passed; `tests/` → 440 passed
  - Constitution Check: роль Reporter не расширена — блок читает сохранённые
    `StageResult.judge`, `.deterministic` и `aggregate_metrics` и ничего не пересчитывает
    (принцип I); исходы судьи, кроме `confirmed`, показаны как нехватка доказательства, а не как
    успех (принцип IV); композит, `findings.py`, JSON-схема и присвоение статуса не менялись
  - Остаётся открытым от T036: бейдж `D` на лестнице намеренно не рисуется — источник по
    умолчанию именно детерминированный, а лестница по дизайн-контракту 002 бессловесная; полный
    источник каждой из шести стадий показан колонкой `Source` в таблице стадий. Оформление
    статуса `INCONCLUSIVE` остаётся отдельным пунктом D-23
