...
## Дельта плана: FIX-11 (T002-8a)

Пустые строковые элементы декларативных маркеров отсекаются на границе
`core/models.py::AttackCandidate`, до передачи кандидата Runner и Oracle.
Файл-аналог — существующий `AttackCandidate`: это текущий входной контракт роли
Attack, поэтому узкая проверка не требует нового `ExpectedEffect` и не переносит
валидацию в Oracle. Проверяются только явно переданные списки `markers`,
`adoption_markers` и `refusal_markers` известного текстового эффекта
