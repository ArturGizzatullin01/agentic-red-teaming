"""memred/core/models.py — типизированные модели, которыми обмениваются все компоненты.

Правило (см. implementation spec, §5): компоненты общаются dataclass'ами, а не
произвольными dict. Единственное место, где dict осознанно допустим — сырые
события трейса (memred/tracing/events.py) и сырые ответы адаптеров, потому что
они по определению непредсказуемой внешней формы.
...
Никогда не схлопывать `None` в `True` — это прямой запрет из spec §14.
"""
