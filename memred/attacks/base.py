...
class AttackBase(ABC):
    """Каждый файл в memred/attacks/ = один класс атаки. Никакой регистрации
    руками — memred/core/config.py подхватывает подклассы по имени family
...
def get_attack(family: str) -> type[AttackBase]:
    if family not in ATTACK_REGISTRY:
        raise KeyError(
            ...
            "Убедись, что модуль memred.attacks.<file> импортирован (см. memred/attacks/__init__.py)."
        )
