"""src/memred/attacks/base.py — контракт атаки, killer feature инструмента.
...
class AttackBase(ABC):
    """Каждый файл в src/memred/attacks/ = один класс атаки. Никакой регистрации
    руками — src/memred/core/config.py подхватывает подклассы по имени family
...
def get_attack(family: str) -> type[AttackBase]:
    if family not in ATTACK_REGISTRY:
        raise KeyError(
            ...
            "Убедись, что модуль memred.attacks.<file> импортирован (см. src/memred/attacks/__init__.py)."
        )
