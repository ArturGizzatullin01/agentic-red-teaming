"""src/memred/reporting/html_report.py — report.html: executive summary,
...
from memred.reporting.findings import Finding, build_findings
from memred.tracing.causal_graph import build_causal_chain, flatten_linear
...
<article class="finding {status_class}">
  ...
  <p class="repro"><strong>Воспроизведение:</strong> <code>memred run --scenario scenarios/{_esc(f.family)}.yaml --output runs/repro-{_esc(f.case_id)}</code>
...
<html lang="ru"><head><meta charset="utf-8"><title>memred report — {_esc(campaign.scenario_id)}</title>
<style>{_CSS}</style></head>
<body><div class="wrap">
  <h1>Agentic Memory Red Teaming — {_esc(campaign.scenario_id)}</h1>
  <p class="muted">run_id={_esc(campaign.run_id)} · попыток: {campaign.attempts}</p>
